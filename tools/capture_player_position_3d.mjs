...
const outputDir = process.env.STADIUM_POSITION_EVIDENCE_DIR ?? "output/stadium-player-position-evidence";
...
async function state(page) {
  return page.locator(".player-position-3d-surface").evaluate((node) => ({
    ...
    progress: Number(node.getAttribute("data-position-progress") ?? "0"),
    complete: node.getAttribute("data-position-complete"),
    x: Number(node.getAttribute("data-position-x")),
    z: Number(node.getAttribute("data-position-z")),
    canvasReady: Boolean(document.querySelector(".player-position-3d-ready")),
    idVisible: Boolean(document.querySelector(".player-position-id-visible")),
    label: document.querySelector(".player-position-id span")?.textContent?.trim() ?? null,
...
async function capture(name, viewport, deviceScaleFactor = 1) {
  ...
  try {
    await page.goto(`${appBaseUrl}/home/position`, { waitUntil: "domcontentloaded" });
    await page.waitForFunction(() => {
      const heading = document.querySelector(".player-position-header h1");
      const surface = document.querySelector(".player-position-3d-surface");
      return heading?.textContent?.trim() === "나의 포지션"
        && surface?.getAttribute("data-render-state") === "READY"
        && Boolean(document.querySelector(".player-position-3d-ready"));
    }, { timeout: 30000 });
    ...
    await page.waitForFunction(() => Number(
      document.querySelector(".player-position-3d-surface")?.getAttribute("data-position-progress") ?? "0",
    ) >= 0.55, { timeout: 60000, polling: 250 });
    const mid = await state(page);
    const midShot = await page.screenshot({ path: `${outputDir}/${name}-mid.png`, fullPage: true });

    await page.waitForFunction(() => (
      document.querySelector(".player-position-3d-surface")?.getAttribute("data-position-complete") === "true"
...
for (const result of results) {
  ...
  if (!valid) {
    console.error(`player position verification failed for ${result.name}`, result);
    process.exitCode = 1;
  }
}

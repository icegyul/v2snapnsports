...
**v1.7 | Frontend Visual + Implementation Lock | 2026-08-27**
...
v1.7은 **논리 Frontend 구조와 Graphite Stadium UI 시각 기준을 함께 잠그는 문서**다. 현재 저장소의 React/React Native/Flutter/웹 프레임워크, router, query library, state library, 3D runtime을 Audit 전에 임의로 교체하거나 확정하지 않는다.
...
- 기본 시각 테마는 **Graphite Stadium UI**다. Dark Navy/near-black blue surface hierarchy는 폐기한다.
- 구형 LCD/저밝기 환경에서도 카드와 컨트롤 경계가 사라지지 않도록 neutral graphite luminance step + visible border를 사용한다.
...
## 4.1 Visual Theme / Display Robustness Contract
- 기본 profile은 `standard_graphite`, 접근성/사용자 선택 profile은 `high_contrast_graphite`다.
- 색상 값은 `SNAPN_SPORTS_V2_FRONTEND_DESIGN_TOKENS_v1.7.yaml` 외부에서 하드코딩하지 않는다.
- Background → Surface → Elevated → Floating의 계층은 hue가 아니라 **명도 단계 + border**로 인지되어야 한다.
- Dark Navy 및 거의 검은 blue/teal surface를 주요 app background/card 계층으로 사용하지 않는다.
- Stadium/Pitch가 화면의 주인공이며 UI surface는 neutral graphite로 뒤로 물러난다. Pitch green은 CTA/선택/핵심 상태에 제한적으로 사용한다.
- 사용자가 `high_contrast_graphite`를 선택하거나 OS increased-contrast signal이 제공되면 surface와 border 명도를 올린다. 휴대폰 모델명/연식으로 자동 판정하지 않는다.
- 주요 화면은 20% 밝기, grayscale, 야외 field-mode 조건을 포함해 수동 QA한다.
- bottom navigation, bottom sheet, modal은 경기장 영상 위에서도 투명 배경에 의존하지 않고 명확한 edge를 유지한다.
...
- 기본 Graphite UI와 High Contrast Graphite는 기능/정보가 완전히 동일해야 한다.
- 텍스트/컨트롤 대비는 semantic token 기준으로 관리하며, core 정보는 muted보다 어두운 토큰을 사용하지 않는다.
...
- FRONTEND_GAP_ANALYSIS_v1.7.md
- FRONTEND_STACK_BINDING_v1.7.md

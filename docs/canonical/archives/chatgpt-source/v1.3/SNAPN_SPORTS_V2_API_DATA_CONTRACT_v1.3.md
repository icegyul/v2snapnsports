...
- EARTHUS_CONTEXT_ENABLED=optional
...
- health, EPTS raw/derived metrics, internal coach notes, private video는 scouting projection에 포함 금지.
...
| Method | Endpoint | Purpose |
| --- | --- | --- |
| GET | /v2/context/events/{event_id} | Earthus 기반 날씨/폭염·한파/대기질/장소 Context projection |

### Normalized context minimum

```json
{
  "status": "FRESH",
  "weather": {"temperature_c":27,"precipitation":null},
  "heat_risk": "LOW",
  "cold_risk": null,
  "air_quality": {"grade":"GOOD"},
  "place_context": {},
  "issued_at": "...",
  "fetched_at": "...",
  "source_summary": "EARTHUS",
  "schema_version": "1"
}
```

- `freshness_state`: FRESH / STALE / PARTIAL / UNAVAILABLE.
- SnapN은 KMA/AirKorea 등 원천 공공데이터 provider를 신규 직접 연결하지 않는다.
- Earthus 실패는 context subresource만 unavailable 처리하며 Event/Training/Match core API를 실패시키지 않는다.
...
| Earthus environment/place context | Earthus via E35 adapter + SnapN normalized cache |

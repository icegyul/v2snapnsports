...
| Phase | 범위 | Gate |
| --- | --- | --- |
| P0 Audit / Freeze | 저장소·DB·Auth·Community·Match/Competition·Media 현행 실사, backup/rollback | 증거 기반 Inventory, write ownership, rollback 검증 |
| P1 Identity / Role / Credential | PLAYER/MANAGER, Guardian invite, Role Preference, Credential Verification, Permission/DataScope | self-selection 권한우회 0건, credential/grant negative tests |
| P2 Legacy Parity / Offline / Privacy | 팀·일정·출석·공지·커뮤니티 Adapter, Offline Journal/Sync, Consent/Data Lifecycle | Legacy parity E2E, offline no-loss, revoke immediate deny |
| P3 Match / Competition Core | Competition/Match/roster/lineup/event/report canonical domain, 역할별 projection | state/event idempotency, referee/coach/player scope tests |
| P4 My Football World / Asset Delivery | Stadium entry, formation/my position, home state, 3D asset manifest/cache, 2D fallback | 실데이터 binding, core asset failure fallback, key info <=3s |
| P5 Stadium DIY / Community Stable / Analytics | Recipe Builder, save/restore, Community parity 유지, product telemetry | recipe reproducibility, analytics privacy separation |
| P6 Training / Tactical / Video / Growth | 센서 없이 동작하는 훈련·전술·영상·Career timeline | EPTS OFF에서도 E2E 완성 |
| P7 Manager Workspaces / Agent / Search | Coach/Team Manager/Club Director/Referee/Agent/Analyst workspace, permission-aware search | 역할별 최소 scope, minor search leakage 0건 |
| P8 Hardening / Feed Experiment | 성능·복구·보안·privacy/export/delete, Feed Intelligence는 opt-in experiment | release blockers 0, feed flag OFF default/experiment audit |
| P9+ Hardware / AI | EPTS, Camera/Vision, Evidence Sports AI | 별도 release approval + PoC/evidence gate |

## 신규 Codex 필수 산출물

```text
docs/v2/
  V2_ROLE_CREDENTIAL_MATRIX.md
  V2_MATCH_COMPETITION_DOMAIN_MAP.md
  V2_MATCH_EVENT_SCHEMA.md
  V2_OFFLINE_SYNC_CONFLICT_POLICY.md
  V2_PRIVACY_LIFECYCLE_MAP.md
  V2_PRODUCT_ANALYTICS_EVENT_SCHEMA.md
  V2_SEARCH_VISIBILITY_MATRIX.md
  V2_COMMUNITY_FEED_EXPERIMENT_PLAN.md
  V2_3D_ASSET_BUNDLE_MANIFEST_SPEC.md
```

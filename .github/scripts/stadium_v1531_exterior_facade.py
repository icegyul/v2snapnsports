...
text = path.read_text()
anchor = "function buildStadium(\n  scene: THREE.Scene,"
if text.count(anchor) != 1:
    raise SystemExit(f"V15.31 facade insertion anchor mismatch: {text.count(anchor)}")

facade = r'''function addExteriorFacade(
  group: THREE.Group,
...
call_anchor = "  const tiers: TierSpec[] = ["
if text.count(call_anchor) != 1:
    raise SystemExit(f"V15.31 facade call anchor mismatch: {text.count(call_anchor)}")
text = text.replace(call_anchor, "  addExteriorFacade(group, geometries, materials, recipe);\n\n" + call_anchor, 1)

...
export function Pack01TrainingPage() { const [response, setResponse] = useState("GOING"); const schedule = domain.getUpcomingSchedule(coach); return <main className="shell-main"><h1>훈련</h1><section aria-label="일정"><h2>다가오는 일정</h2>{schedule.map((item) => <Link key={item.id} to={item.targetRoute}>{item.type === "TRAINING" ? "훈련" : "경기"} · {item.startAt}</Link>)}</section><article><h2>다음 훈련</h2><p>{training.objective}</p><p>참가 상태: {response}</p><button onClick={() => setResponse("LATE")}>늦게 참여</button><Link to={`/training/${training.id}`}>훈련 상세</Link></article></main>; }

...
export function createStadiumWebglRenderer(
  ...
  const drawCrowd = () => {
    ...
    for (let row = 0; row < rows; row += 1) {
      ...
      const yNorm = portrait ? 0.32 + t * 0.27 : 0.31 + t * 0.28;
      for (let s = 0; s < seatsPerRow; s += 1) {
        ...
        const y = (yNorm + edge * edge * (portrait ? 0.018 : 0.014) + (hash(seed, 3) - 0.5) * 0.003) * canvas.height;
        const bright = hash(seed, 4);
        const blue = hash(seed, 5) > 0.90;
        const alpha = bright > 0.97 ? 0.22 : bright > 0.84 ? 0.11 : 0.055;
        ctx.fillStyle = blue
          ? `rgba(58,132,220,${alpha})`
          : bright > 0.97
            ? `rgba(220,218,207,${alpha})`
            : `rgba(145,151,157,${alpha})`;
        const size = (0.8 + t * 0.45) * pixelRatio;
  ...
  const render = (orbit: number, zoom: number) => {
    base.render(orbit, portrait ? Math.max(1.03, zoom) : Math.max(1.08, zoom));
    const sw = frame.width;
    const sh = frame.height;
    const sy = portrait ? Math.round(sh * 0.13) : Math.round(sh * 0.07);
    const cropBottom = portrait ? 0.37 : 0.285;
    ...
    ctx.save();
    ctx.globalCompositeOperation = "screen";
    ctx.globalAlpha = portrait ? 0.032 : 0.040;
    ...
    ctx.save();
    ctx.globalCompositeOperation = "screen";
    const radius = Math.max(canvas.width, canvas.height) * (portrait ? 0.11 : 0.085);
    drawLight(canvas.width * 0.10, canvas.height * 0.045, radius, portrait ? 0.08 : 0.12);
    drawLight(canvas.width * 0.90, canvas.height * 0.045, radius, portrait ? 0.08 : 0.12);
    drawLight(canvas.width * 0.24, canvas.height * 0.12, radius * 0.68, portrait ? 0.04 : 0.055);
    drawLight(canvas.width * 0.76, canvas.height * 0.12, radius * 0.68, portrait ? 0.04 : 0.055);
    ctx.restore();
    ...
    vignette.addColorStop(1, portrait ? "rgba(0,0,0,0.13)" : "rgba(0,0,0,0.12)");
    ctx.save();

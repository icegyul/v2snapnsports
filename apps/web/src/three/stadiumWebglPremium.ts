...
const C = {
  grassA:[0.016,0.30,0.060] as Rgb, grassB:[0.010,0.205,0.040] as Rgb,
  white:[0.94,0.97,1.0] as Rgb, seatDark:[0.006,0.018,0.060] as Rgb,
  seat:[0.012,0.10,0.36] as Rgb, seatBright:[0.020,0.28,0.88] as Rgb,
  concrete:[0.13,0.16,0.20] as Rgb, dark:[0.018,0.024,0.034] as Rgb,
  roof:[0.16,0.19,0.24] as Rgb, steel:[0.34,0.40,0.47] as Rgb,
  black:[0.002,0.004,0.008] as Rgb, led:[0.025,0.32,1.0] as Rgb,
  ledWhite:[0.80,0.94,1.0] as Rgb, warm:[1.0,0.90,0.64] as Rgb,
  crowd:[0.42,0.54,0.78] as Rgb, crowdWhite:[0.82,0.88,0.95] as Rgb,
} as const;
...
function addTier(m:Mesh,t:Tier,seg:number,lightMode:boolean){
  ...
  for(let r=0;r<t.rows;r++){
    ...
    band(m,ix,iz,ox,oz,y,y+t.rise*.11,seg,c);if(r%2===0)wall(m,ox,oz,y-.20,y+.12,seg,C.dark);
    for(let i=0;i<seats;i++){if((i+r*7)%19===0)continue;const a=(i+.5)/seats*Math.PI*2,rx=ix+t.depth*.53,rz=iz+t.depth*.38;const crowd=(i+r)%13===0?C.crowdWhite:((i+r)%4===0?C.crowd:C.seatBright);box(m,[Math.cos(a)*rx,y+.38,Math.sin(a)*rz],[.14,.46,.14],crowd)}
  }
}
...
function addFacade(m:Mesh,seg:number){wall(m,113,83,-1.4,5.6,seg,C.dark);band(m,109,79,115,85,5.2,7.0,seg,C.concrete);for(let i=0;i<64;i++){const a=i/64*Math.PI*2;box(m,[Math.cos(a)*111.8,2.5,Math.sin(a)*82],[1.05,3.2,.24],i%5===0?C.warm:C.black)}box(m,[0,1.6,82],[22,3.2,2],C.black);box(m,[0,4.4,81.4],[24,.34,1],C.led)}
function addRoof(m:Mesh,seg:number){band(m,94,68,117,86,38.2,43.2,seg,C.roof,.76);band(m,91,65,94,68,37.7,38.5,seg,C.black,.76);for(let i=0;i<56;i++){const a=i/56*Math.PI*2;if(Math.sin(a)>.78)continue;const x=Math.cos(a),z=Math.sin(a);box(m,[x*106,40.9,z*78],[.34,5.5,.34],C.steel);box(m,[x*97,38.4,z*70],[.26,2.1,.26],C.steel)}}
...
function build(mode:Exclude<CoreVisualMode,"STATIC">){const m=mesh(),seg=mode==="FULL"?160:mode==="FAST"?120:80,light=mode==="LIGHT";addPitch(m,seg);addFacade(m,seg);addTier(m,{ix:61,iz:43,rows:13,depth:1.22,rise:.70,y:1,seats:100,c0:C.seatDark,c1:C.seat},seg,light);addAisles(m,61,43,77,55,1,9.5,32);addRibbon(m,77.5,55.5,10.0,seg);addTier(m,{ix:79,iz:57,rows:13,depth:1.10,rise:.74,y:11,seats:112,c0:C.seat,c1:C.seatBright},seg,light);addAisles(m,79,57,93,67,11,20.5,36);addRibbon(m,93.5,67.5,21.0,seg);addTier(m,{ix:95,iz:69,rows:12,depth:1.08,rise:.76,y:22,seats:120,c0:C.seatDark,c1:C.seat},seg,light);addAisles(m,95,69,108,78,22,31,40);addScreens(m);addRoof(m,seg);addLights(m);return m}
...
function createProgram(gl:WebGLRenderingContext){
  ...
  const fs=shader(gl,gl.FRAGMENT_SHADER,`precision mediump float;varying vec3 vPos;varying vec3 vNormal;varying vec3 vColor;uniform vec3 uCamera;uniform vec3 uSunDir;
float sat(float x){return clamp(x,0.0,1.0);} vec3 lamp(vec3 lp,vec3 tint,float power){vec3 L=lp-vPos;float dist=max(length(L),1.0);vec3 l=normalize(L);float nd=sat(dot(vNormal,l));float att=1.0/(1.0+0.00017*dist*dist);vec3 V=normalize(uCamera-vPos);vec3 H=normalize(l+V);float spec=pow(sat(dot(vNormal,H)),38.0)*0.62;return tint*(nd+spec)*att*power;}
void main(){vec3 N=normalize(vNormal);vec3 V=normalize(uCamera-vPos);float sun=sat(dot(N,normalize(uSunDir)));float up=sat(N.y);float radius=length(vPos.xz);float pitchMask=(1.0-smoothstep(38.0,70.0,radius))*step(vPos.y,1.2);float grassMask=step(vColor.r*2.0,vColor.g)*step(vColor.b*1.2,vColor.g)*pitchMask;float standMask=smoothstep(55.0,82.0,radius)*step(vPos.y,36.0);
vec3 ambient=vec3(.032,.046,.072)*(0.72+up*.36);vec3 c=vColor*(ambient+sun*.22);
c+=vColor*lamp(vec3(-104.0,46.0,-75.0),vec3(.80,.90,1.0),3.00);c+=vColor*lamp(vec3(104.0,46.0,-75.0),vec3(.80,.90,1.0),3.00);c+=vColor*lamp(vec3(-104.0,46.0,75.0),vec3(.78,.88,1.0),2.70);c+=vColor*lamp(vec3(104.0,46.0,75.0),vec3(.78,.88,1.0),2.70);
float centerPool=exp(-0.00060*(vPos.x*vPos.x+vPos.z*vPos.z));c+=vColor*vec3(.42,.48,.56)*centerPool*(.60+.40*up);float mow=.96+.06*sin(vPos.x*.42)+.03*sin(vPos.z*.72);c*=mix(1.0,mow,grassMask);c+=vec3(.025,.10,.032)*grassMask*centerPool*.90;
float rim=pow(1.0-sat(dot(V,N)),3.0);c+=vec3(.08,.16,.30)*rim*(.16+.20*standMask);float mx=max(max(vColor.r,vColor.g),vColor.b);float emiss=smoothstep(.72,.96,mx);c+=vColor*emiss*1.45;float ao=1.0-.14*standMask*(1.0-up);c*=ao;
float dist=length(uCamera-vPos);float fog=smoothstep(205.0,430.0,dist);vec3 fogC=vec3(.010,.020,.036);c=mix(c,fogC,fog*.30);c=c/(c+vec3(.82));c=pow(c,vec3(.82));gl_FragColor=vec4(c,1.0);}`);
...
export function createStadiumWebglRenderer(canvas:HTMLCanvasElement,mode:Exclude<CoreVisualMode,"STATIC">):StadiumWebglRenderer|null{
  ...
  const maybeGl=canvas.getContext("webgl",{alpha:true,antialias:true,depth:true,premultipliedAlpha:false,powerPreference:mode==="FULL"?"high-performance":"default"}) as WebGLRenderingContext|null;if(!maybeGl)return null;const gl:WebGLRenderingContext=maybeGl;
  ...
  function render(o:number,z0:number){const a=vw/vh,portrait=a<1,z=Math.min(1.18,Math.max(.92,z0)),oR=o*Math.PI/180,r=(portrait?198:150)/z,h=(portrait?82:54)/z,e:Vec3=[Math.sin(oR)*r,h,Math.cos(oR)*r],v=look(e,[0,8,0],[0,1,0]),pr=persp((portrait?61:52)*Math.PI/180,a,.7,520),mvp=mul(pr,v);gl.clearColor(0,0,0,0);gl.clearDepth(1);gl.clear(gl.COLOR_BUFFER_BIT|gl.DEPTH_BUFFER_BIT);gl.useProgram(p);bind(pb,pos);bind(nb,nor);bind(cb,col);gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER,ib);gl.uniformMatrix4fv(uMvp,false,mvp);gl.uniform3f(uCamera,e[0],e[1],e[2]);gl.uniform3f(uSun,-.24,.94,.30);gl.drawElements(gl.TRIANGLES,m.indices.length,uint?gl.UNSIGNED_INT:gl.UNSIGNED_SHORT,0)}
